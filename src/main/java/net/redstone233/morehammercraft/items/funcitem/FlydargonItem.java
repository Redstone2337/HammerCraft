...
public class FlydargonItem extends Item {
    ...
    public FlydargonItem(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient()) {
            float randomFloat = random.nextFloat(10) + 1;
            if (a != 0) {
                world.createExplosion(context.getPlayer(),context.getBlockPos().getX(),context.getBlockPos().getY(),context.getBlockPos().getZ(),randomFloat,true, World.ExplosionSourceType.BLOCK);
            }
        }
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        if (Screen.hasAltDown()) {
            tooltip.add(Text.translatable("item.mhc.flydragon_studio.alt.tooltip"));
        } else {
            tooltip.add(Text.translatable("item.mhc.flydragon_studio.tooltip"));
        }
