...
public class VampireHammer extends Item {
    private static int ATTACK_DAMAGE_MODIFIER_VALUE = 20;
    private static float ATTACK_SPEED_MODIFIER_VALUE = -0.5F;
    public static final float MINING_SPEED_MULTIPLIER = 4.5F;
    ...
    public VampireHammer(Settings settings) {
        super(settings);
    }
    ...
    public static AttributeModifiersComponent createAttributeModifiers() {
        return AttributeModifiersComponent.builder()
                .add(
                        ...
                        new EntityAttributeModifier(BASE_ATTACK_DAMAGE_MODIFIER_ID, getAttackDamageModifierValue(), EntityAttributeModifier.Operation.ADD_VALUE),
                        AttributeModifierSlot.MAINHAND
                )
                .add(
                        EntityAttributes.ATTACK_SPEED,
                        new EntityAttributeModifier(BASE_ATTACK_SPEED_MODIFIER_ID, getAttackSpeedModifierValue()*MINING_SPEED_MULTIPLIER, EntityAttributeModifier.Operation.ADD_VALUE),
                        AttributeModifierSlot.MAINHAND
                )
                .build();
    }
    ...
    public static ToolComponent createToolComponent() {
        return new ToolComponent(List.of(), getAttackSpeedModifierValue()*MINING_SPEED_MULTIPLIER, 2);
    }

    public static double healAmounts = 2.0;

    @Override
    public boolean postHit(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        ServerWorld serverWorld = (ServerWorld)attacker.getWorld();
        if (attacker instanceof PlayerEntity) {
            double healAmount = healAmounts;
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        ...
        if (Screen.hasAltDown()) {
            tooltip.add(Text.translatable("item.mhc.vampire.alt.tooltip",healAmounts,ATTACK_DAMAGE_MODIFIER_VALUE,ATTACK_DAMAGE_MODIFIER_VALUE));
        } else {
    ...
    public static void setHealAmount(double amount){
        healAmounts = amount;
    }

    public static void setAttackDamageModifierValue(int attackDamageModifierValue) {
        ATTACK_DAMAGE_MODIFIER_VALUE = attackDamageModifierValue;
    }

    public static void setAttackSpeedModifierValue(float attackSpeedModifierValue) {
        ATTACK_SPEED_MODIFIER_VALUE = attackSpeedModifierValue;
    }

    public static double getHealAmounts() {
        return healAmounts;
    }

    public static float getAttackSpeedModifierValue() {
        return ATTACK_SPEED_MODIFIER_VALUE;
    }

    public static int getAttackDamageModifierValue() {
        return ATTACK_DAMAGE_MODIFIER_VALUE;
    }
}
