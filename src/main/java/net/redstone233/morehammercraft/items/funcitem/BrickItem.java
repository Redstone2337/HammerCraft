...
public class BrickItem extends Item {
    ...
    public ActionResult useOnEntity(ItemStack stack, PlayerEntity user, LivingEntity entity, Hand hand) {
        if (user != null && hand == Hand.MAIN_HAND){
            ...
            entity.addStatusEffect(new StatusEffectInstance(StatusEffects.GLOWING,10,5,true,true,true));
    ...
    public float getBonusAttackDamage(Entity target, float baseAttackDamage, DamageSource damageSource) {
        ...
        return ATTACK_DAMAGE_MODIFIER_VALUE * MINING_SPEED_MULTIPLIER;
    }
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        if (Screen.hasAltDown()) {
            tooltip.add(Text.translatable("item.mhc.brick.tooltip"));
        } else {
            tooltip.add(Text.translatable("item.mhc.brick.alt.tooltip"));
        }
