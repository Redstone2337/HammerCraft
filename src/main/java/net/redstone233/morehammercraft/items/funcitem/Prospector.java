    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient()) {
            ...
            if (!Screen.hasShiftDown()) {
                    ...
                    for (int j = 0; j < j1; j++) {
                        for (int k = 0; k < k1; k++) {
                            ...
                            if (isRightblock(blockState)) {
                                player.sendMessage(Text.translatable("prospector.mhc.success",pos.getX(),pos.getY(),pos.getZ(),name));
                                foundblock = true;
                                break;
                            }
                        }
                ...
                if (!foundblock) {
                    player.sendMessage(Text.translatable("prospector.mhc.error"));
                }
            } else {
                for (int i = 0; i <= pos.getY() + 64; i++) {
                    ...
                    if (isRightblock(blockState)) {
                        player.sendMessage(Text.translatable("prospector.mhc.success",name,pos.getX(),pos.getY(),pos.getZ()));
                        foundblock = true;
                        break;
                    }
                }
