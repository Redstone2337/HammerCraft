...
public class WindowsItem extends Item {
    ...
    private static final int ATTACK_DAMAGE_MODIFIER_VALUE = 499;
    ...
    public WindowsItem(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient()) {
            world.createExplosion(context.getPlayer(),context.getBlockPos().getX(),context.getBlockPos().getY(),context.getBlockPos().getZ(),6.0F,true, World.ExplosionSourceType.BLOCK);
        }
    ...
    public ActionResult useOnEntity(ItemStack stack, PlayerEntity user, LivingEntity entity, Hand hand) {
        if (hand != null) {
            ...
            entity.addStatusEffect(new StatusEffectInstance(StatusEffects.SLOWNESS,200,100,true,true,true));
            user.addStatusEffect(new StatusEffectInstance(StatusEffects.JUMP_BOOST,300,2,true,true,true));
            return ActionResult.SUCCESS;
        }
    }
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        if (Screen.hasAltDown()) {
            tooltip.add(Text.translatable("item.mhc.windows.tooltip"));
        } else {
            tooltip.add(Text.translatable("item.mhc.windows.alt.tooltip"));
        }
